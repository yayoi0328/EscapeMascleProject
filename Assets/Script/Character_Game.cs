...
public class Character_Game : MonoBehaviour
{

    private Image character;
    //   private int[] startcharacter = { 0, 1, 3, 2, 4 };
    // Start is called before the first frame update
    private int[] CharacterArray = { 0, 1, 3, 1, 4 };
    ...
    void Update()
    {
        ...
        Num = GameObject.Find("MessageUI").GetComponent<Message_Game>().messageNum;

        if (Num < CharacterArraySize)
        {
            Displaycharacter(CharacterArray[Num]);
        }
        if (false == GameObject.Find("MessageUI").GetComponent<Message_Game>().transform.GetChild(0).gameObject.activeSelf)
        {
            character.enabled = false;
